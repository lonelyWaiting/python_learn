...
# 查找是在表中查找，因此查找应该被定义为classmethod
# 插入是将实例数据插入表中，因此save操作由实例调用
class ModelMetaclass(type):
    def __new__(cls,name,bases,attrs):
        ...
        # get all field and primary key name
        mappings = dict()   # recode all key-value pair
        fields = []         # recode all field of not primary_key
        primaryKey = None   
        ...
        # mapping save all field attr
        for k in mappings.keys():
            attrs.pop(k)
        
        escaped_fields = list(map(lambda f : '`%s`' % f, fields))
        attrs['__mappings__'] = mappings        # save attr <=> column mapping relation
...
class Model(dict,metaclass=ModelMetaclass):
    ...
    def getValueOrDefault(self,key):
        value = getattr(self,key,None)  #third param is default value if get failed
        if value is None:
